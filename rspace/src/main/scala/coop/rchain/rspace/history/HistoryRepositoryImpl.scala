...
final case class HistoryRepositoryImpl[F[_]: Sync: Parallel, C, P, A, K](
  ...
  def checkpointWithRetry(
    ...
    def toHex(hash: Blake2b256Hash) = hash.bytes.take(6).toHex
    ...
    println(s"DISTRIBUTION size:\n${chDistrib.toSeq.size}")
    ...
    val mkCP = for {
      _              <- println(s"CHECKPOINT root: ${toHex(history.root)}").pure[F]
      _              <- println(s"   changes size: ${actions.size}").pure[F]
      historyActions <- List(actions).parFlatTraverse(transformAndStore)
      _              <- println(s" historyActions: ${historyActions.size}").pure[F]
    ...
    for {
      next <- withRetry[F, History[F]](
               mkCP,
               maxRetries, {
                 case _ => true // next.root != expectedRoot
               }, {
                 case (next, n) =>
                   println(
